...
var _xmlParsers = require("./xml-parsers");
...
class Client {
  ...
  getRequestOptions(opts) {
    ...
    if (headers) {
      ...
      _lodash.default.map(headers, (v, k) => reqOptions.headers[k.toLowerCase()] = v);
  ...
  logHTTP(reqOptions, response, err) {
    ...
    var logHeaders = headers => {
      _lodash.default.forEach(headers, (v, k) => {
        if (k == 'authorization') {
  ...
  makeRequestStream(options, stream, sha256sum, statusCodes, region, returnResponse, cb) {
    ...
    var _makeRequest = (e, region) => {
      ...
      if (!this.anonymous) {
        ...
        this.checkAndRefreshCreds();
        var authorization = (0, _signing.signV4)(reqOptions, this.accessKey, this.secretKey, region, date);
      ...
      var req = this.transport.request(reqOptions, response => {
        if (!statusCodes.includes(response.statusCode)) {
          ...
          (0, _helpers.pipesetup)(response, errorTransformer).on('error', e => {
      ...
      let pipe = (0, _helpers.pipesetup)(stream, req);
      pipe.on('error', e => {
  ...
  getBucketRegion(bucketName, cb) {
    ...
    var extractRegion = response => {
      ...
      (0, _helpers.pipesetup)(response, transformer).on('error', cb).on('data', data => {
        if (data) region = data;
      }).on('end', () => {
  ...
  listBuckets(cb) {
    ...
    this.makeRequest({
      ...
      var buckets;
      (0, _helpers.pipesetup)(response, transformer).on('data', result => buckets = result).on('error', e => cb(e)).on('end', () => cb(null, buckets));
    });
  ...
  listIncompleteUploads(bucket, prefix, recursive) {
    ...
    readStream._read = () => {
      ...
      this.listIncompleteUploadsQuery(bucket, prefix, keyMarker, uploadIdMarker, delimiter).on('error', e => readStream.emit('error', e)).on('data', result => {
        ...
        _async.default.eachSeries(result.uploads, (upload, cb) => {
  ...
  fGetObject(bucketName, objectName, filePath, getOpts = {}, cb) {
    ...
    _async.default.waterfall([cb => this.statObject(bucketName, objectName, getOpts, cb), (result, cb) => {
      ...
      _fs.default.stat(partFile, (e, stats) => {
        var offset = 0;
      ...
      (0, _helpers.pipesetup)(downloadStream, partFileStream).on('error', e => cb(e)).on('finish', cb);
    }, cb => _fs.default.stat(partFile, cb), (stats, cb) => {
  ...
  fPutObject(bucketName, objectName, filePath, metaData, callback) {
    ...
    _async.default.waterfall([cb => _fs.default.stat(filePath, cb), (stats, cb) => {
      ...
      if (uploadId) return this.listParts(bucketName, objectName, uploadId, (e, etags) => cb(e, uploadId, etags)); // there was no previous upload, initiate a new one
      ...
      var multipart = true;
      var uploader = this.getUploader(bucketName, objectName, metaData, multipart); // convert array to object to make things easy
  ...
  putObject(bucketName, objectName, stream, size, metaData, callback) {
    ...
    let chunker = new _blockStream.default({
      size,
      zeroPadding: false
  ...
  copyObjectV1(arg1, arg2, arg3, arg4, arg5) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformer).on('error', e => cb(e)).on('data', data => cb(null, data));
    });
  }
  /**
  ...
  copyObjectV2(sourceConfig, destConfig, cb) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformer).on('error', e => cb(e)).on('data', data => {
  ...
  listObjectsQuery(bucketName, prefix, marker, listQueryOpts = {}) {
    ...
    const queries = []; // escape every value in query string, except maxKeys
  ...
  getBucketPolicy(bucketName, cb) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformers.getConcater()).on('data', data => policy = data).on('error', cb).on('end', () => {
  ...
  presignedPostPolicy(postPolicy, cb) {
    ...
    this.getBucketRegion(postPolicy.formData.bucket, (e, region) => {
      ...
      postPolicy.policy.conditions.push(["eq", "$x-amz-credential", this.accessKey + "/" + (0, _helpers.getScope)(region, date)]);
      postPolicy.formData['x-amz-credential'] = this.accessKey + "/" + (0, _helpers.getScope)(region, date);
  ...
  initiateNewMultipartUpload(bucketName, objectName, metaData, cb) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformer).on('error', e => cb(e)).on('data', uploadId => cb(null, uploadId));
    });
  ...
  completeMultipartUpload(bucketName, objectName, uploadId, etags, cb) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformer).on('error', e => cb(e)).on('data', result => {
        if (result.errCode) {
  ...
  getBucketNotification(bucketName, cb) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformer).on('data', result => bucketNotification = result).on('error', e => cb(e)).on('end', () => cb(null, bucketNotification));
    });
  ...
  getBucketVersioning(bucketName, cb) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformers.bucketVersioningTransformer()).on('data', data => {
        versionConfig = data;
      }).on('error', cb).on('end', () => {
  ...
  getBucketTagging(bucketName, cb) {
    ...
    this.makeRequest(requestOptions, '', [200], '', true, (e, response) => {
      ...
      if (e) return cb(e);
      let tagsList;
      (0, _helpers.pipesetup)(response, transformer).on('data', result => tagsList = result).on('error', e => cb(e)).on('end', () => cb(null, tagsList));
    });
  }
  ...
  getBucketLifecycle(bucketName, cb) {
    ...
    this.makeRequest(requestOptions, '', [200], '', true, (e, response) => {
      ...
      if (e) return cb(e);
      let lifecycleConfig;
      (0, _helpers.pipesetup)(response, transformer).on('data', result => lifecycleConfig = result).on('error', e => cb(e)).on('end', () => cb(null, lifecycleConfig));
    });
  }
  ...
  setObjectLockConfig(bucketName, lockConfigOpts = {}, cb) {
    ...
    const configKeys = Object.keys(lockConfigOpts); // Check if keys are present and all keys are present.
  ...
  getObjectLockConfig(bucketName, cb) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformers.objectLockTransformer()).on('data', data => {
  ...
  getObjectRetention(bucketName, objectName, getOpts, cb) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformers.objectRetentionTransformer()).on('data', data => {
  ...
  setBucketEncryption(bucketName, encryptionConfig, cb) {
    ...
    let builder = new _xml2js.default.Builder({
  ...
  getBucketEncryption(bucketName, cb) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformers.bucketEncryptionTransformer()).on('data', data => {
  ...
  getBucketReplication(bucketName, cb) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformers.replicationConfigTransformer()).on('data', data => {
  ...
  getObjectLegalHold(bucketName, objectName, getOpts = {}, cb) {
    ...
    this.makeRequest({
      ...
      (0, _helpers.pipesetup)(response, transformers.objectLegalHoldTransformer()).on('data', data => {
  ...
  async setCredentialsProvider(credentialsProvider) {
    if (!(credentialsProvider instanceof _CredentialProvider.default)) {
  ...
  uploadPartCopy(partConfig, cb) {
    ...
    return this.makeRequest(requestOptions, '', [200], '', true, (e, response) => {
      ...
      if (e) return cb(e);
      (0, _helpers.pipesetup)(response, transformers.uploadPartTransformer()).on('data', data => {
        partCopyResult = data;
      }).on('error', cb).on('end', () => {
  ...
  composeObject(destObjConfig = {}, sourceObjList = [], cb) {
    ...
    return Promise.all(sourceObjStats).then(srcObjectInfos => {
      const validatedStats = srcObjectInfos.map((resItemStat, index) => {
        ...
        let srcCopySize = resItemStat.size; // Check if a segment is specified, and if so, is the
      ...
      const splitPartSizeList = validatedStats.map((resItemStat, idx) => {
        const calSize = (0, _helpers.calculateEvenSplits)(srcObjectSizes[idx], sourceObjList[idx]);
        return calSize;
      });
      ...
      const performUploadParts = uploadId => {
        ...
        _async.default.map(uploadList, me.uploadPartCopy.bind(me), (err, res) => {
          if (err) {
  ...
  selectObjectContent(bucketName, objectName, selectOpts = {}, cb) {
    ...
    if (!_lodash.default.isEmpty(selectOpts)) {
      if (!(0, _helpers.isString)(selectOpts.expression)) {
        throw new TypeError('sqlExpression should be of type "string"');
      }

      if (!_lodash.default.isEmpty(selectOpts.inputSerialization)) {
        if (!(0, _helpers.isObject)(selectOpts.inputSerialization)) {
      ...
      if (!_lodash.default.isEmpty(selectOpts.outputSerialization)) {
        if (!(0, _helpers.isObject)(selectOpts.outputSerialization)) {
    ...
    this.makeRequest({
      ...
      if (e) return cb(e);
      let selectResult;
      (0, _helpers.pipesetup)(response, transformers.selectObjectContentTransformer()).on('data', data => {
...
Client.prototype.selectObjectContent = (0, _helpers.promisify)(Client.prototype.selectObjectContent);
